...
opts_chunk$set(sanitize=TRUE)
opts_chunk$set(dev=miaccs["carpools.device",3]) # ImageMagick needs to be installed!
opts_chunk$set(dpi=as.numeric(miaccs["carpools.dpi",3]))
...
analysis.name = stri_replace_all_fixed(miaccs["assay.name",3], " ", "_")
opts_chunk$set(fig.path=paste(datapath,"/",analysis.name, "/", sep=""))
...
# name of bowtie2 index file without any extension (no .bt2 etc)
...
sensitivity = as.character(miaccs["carpools.bt2.additional",3])
...
a.annotate = unlist(strsplit(stri_replace_all_fixed(miaccs["carpools.bm.attributes",3]," ","", fixed=TRUE), ",", fixed=TRUE))
...
#### DATA ANALYSIS
# Targeting Controls
controls.target = as.character(unlist(strsplit(stri_replace_all_fixed(miaccs["carpools.pos",3]," ", "", fixed=TRUE), ",", fixed=TRUE)))
if(controls.target == "NULL" || controls.target == "none")
  {
  controls.target = NULL
  }

# Non-Targeting Controls
controls.nontarget = as.character(unlist(strsplit(stri_replace_all_fixed(miaccs["carpools.non-targeting",3]," ", "", fixed=TRUE), ",", fixed=TRUE)))
if(controls.nontarget == "NULL" || controls.nontarget == "none")
  {
...
if(miaccs["assay.plasmids.image",3] != "none" && !is.na(miaccs["assay.plasmids.image",3]))
  {
    ...
    for(i in 1:length(plasmid.image))
      {
        cat("\n")
...
if(miaccs["cell.setup.image",3] != "none"&& !is.na(miaccs["cell.setup.image",3]))
  {
    ...
    for(i in 1:length(screen.image))
      {
        cat("\n")
...
if(identical(mapping, TRUE))
  {
    # Extract Data?
   fileCONTROL1 = data.extract(scriptpath, datapath, fastqfile=fileCONTROL1, extract, seq.pattern, maschine.pattern, createindex, bowtie2file, referencefile, mapping, reversecomplement, threads, bowtieparams, sensitivity, match)
        
   fileCONTROL2 = data.extract(scriptpath, datapath, fastqfile=fileCONTROL2, extract, seq.pattern, maschine.pattern, createindex, bowtie2file, referencefile, mapping, reversecomplement, threads, bowtieparams, sensitivity, match)
        
   fileTREAT1 = data.extract(scriptpath, datapath, fastqfile=fileTREAT1, extract, seq.pattern, maschine.pattern, createindex, bowtie2file, referencefile, mapping, reversecomplement, threads, bowtieparams, sensitivity, match)
        
   fileTREAT2 = data.extract(scriptpath, datapath, fastqfile=fileTREAT2, extract, seq.pattern, maschine.pattern, createindex, bowtie2file, referencefile, mapping, reversecomplement, threads, bowtieparams, sensitivity, match)
...
__A brief look into the data passed on to the analysis scripts:__  

`r # make randoms for brief stats table`
`r sample.sgrna = sample(1:nrow(CONTROL1),10)`

__`r d.CONTROL1`__  
__`r fileCONTROL1`__
`r kable(CONTROL1[sample.sgrna,])`

__`r d.CONTROL2`__  
__`r fileCONTROL2`__
`r kable(CONTROL2[sample.sgrna,])`

__`r d.TREAT1`__  
__`r fileTREAT1`__
`r kable(TREAT1[sample.sgrna,])`

__`r d.TREAT2`__  
__`r fileTREAT2`__
`r kable(TREAT2[sample.sgrna,])`
...
CONTROL1.g=aggregate.to.genes(CONTROL1, agg.function=sum, extractpattern = g.extractpattern)
CONTROL2.g=aggregate.to.genes(CONTROL2, agg.function=sum, extractpattern = g.extractpattern)
TREAT1.g=aggregate.to.genes(TREAT1, agg.function=sum, extractpattern = g.extractpattern)
TREAT2.g=aggregate.to.genes(TREAT2, agg.function=sum, extractpattern = g.extractpattern)

# Add genes names to normal count file (if not present)
CONTROL1=aggregate.to.genes(CONTROL1, extractpattern = g.extractpattern, type="annotate")
CONTROL2=aggregate.to.genes(CONTROL2, extractpattern = g.extractpattern, type="annotate")
TREAT1=aggregate.to.genes(TREAT1, extractpattern = g.extractpattern,type="annotate")
TREAT2=aggregate.to.genes(TREAT2, extractpattern = g.extractpattern, type="annotate")
...
# output to report
kable(combined.stats)

# output to file
write.xlsx(combined.stats, file=paste(datapath, paste(analysis.name, "STATS.xls", sep="_"), sep="/"), sheetName="Combined Stats", row.names=FALSE)
```
...
# output to file
write.xlsx(U1.unmapped, file=paste(datapath, paste(analysis.name, "DROPOUT.xls", sep="_"), sep="/"), sheetName=d.CONTROL1, row.names=FALSE)
write.xlsx(U2.unmapped, file=paste(datapath, paste(analysis.name, "DROPOUT.xls", sep="_"), sep="/"), sheetName=d.CONTROL2, append=TRUE, row.names=FALSE)
write.xlsx(T1.unmapped, file=paste(datapath, paste(analysis.name, "DROPOUT.xls", sep="_"), sep="/"), sheetName=d.TREAT1, append=TRUE, row.names=FALSE)
write.xlsx(T2.unmapped, file=paste(datapath, paste(analysis.name, "DROPOUT.xls", sep="_"), sep="/"), sheetName=d.TREAT2, append=TRUE, row.names=FALSE)
...
write.xlsx(combined.stats.mean, file=paste(datapath, paste(analysis.name, "STATS.xls", sep="_"), sep="/"), sheetName="Mean Read Count", append=TRUE ,row.names=FALSE)
write.xlsx(combined.stats.median, file=paste(datapath, paste(analysis.name, "STATS.xls", sep="_"), sep="/"), sheetName="Median Read Count", append=TRUE, row.names=FALSE)
write.xlsx(combined.stats.min, file=paste(datapath, paste(analysis.name, "STATS.xls", sep="_"), sep="/"), sheetName="Minimum Read Count", append=TRUE, row.names=FALSE)
write.xlsx(combined.stats.max, file=paste(datapath, paste(analysis.name, "STATS.xls", sep="_"), sep="/"), sheetName="Maximum Read Count", append=TRUE, row.names=FALSE)
...
plot.read.distribution(CONTROL1, fullmatchcolumn=fullmatchcolumn,breaks=200, title=d.CONTROL1, xlab="log2 Readcount", ylab="# sgRNAs",statistics=plot.statistic) 

plot.read.distribution(CONTROL1, fullmatchcolumn=fullmatchcolumn,breaks=200, title=d.CONTROL1, xlab="log2 Readcount", ylab="# sgRNAs",statistics=plot.statistic, type="whisker") 

plot.read.distribution(CONTROL2, fullmatchcolumn=fullmatchcolumn,breaks=200, title=d.CONTROL2, xlab="log2 Readcount", ylab="# sgRNAs",statistics=plot.statistic) 

plot.read.distribution(CONTROL2, fullmatchcolumn=fullmatchcolumn,breaks=200, title=d.CONTROL2, xlab="log2 Readcount", ylab="# sgRNAs",statistics=plot.statistic, type="whisker") 
# add new page


plot.read.distribution(TREAT1, fullmatchcolumn=fullmatchcolumn,breaks=200, title=d.TREAT1, xlab="log2 Readcount", ylab="# sgRNAs",statistics=plot.statistic) 

plot.read.distribution(TREAT1, fullmatchcolumn=fullmatchcolumn,breaks=200, title=d.TREAT1, xlab="log2 Readcount", ylab="# sgRNAs",statistics=plot.statistic, type="whisker") 

plot.read.distribution(TREAT2, fullmatchcolumn=fullmatchcolumn,breaks=200, title=d.TREAT2, xlab="log2 Readcount", ylab="# sgRNAs",statistics=plot.statistic) 

plot.read.distribution(TREAT2, fullmatchcolumn=fullmatchcolumn,breaks=200, title=d.TREAT2, xlab="log2 Readcount", ylab="# sgRNAs",statistics=plot.statistic, type="whisker") 
...
#par(mfrow=c(4,1))
plot.read.depth(datasets = list(CONTROL1,CONTROL2,TREAT1,TREAT2), namecolumn=namecolumn ,fullmatchcolumn=fullmatchcolumn, dataset.names=list(d.CONTROL1,d.CONTROL2,d.TREAT1,d.TREAT2), extractpattern=g.extractpattern, xlab="Genes", ylab="Read Count per sgRNA",statistics=plot.statistic, labelgenes = NULL, controls.target = controls.target, controls.nontarget=controls.nontarget)
...
control1.readspergene = plot.reads.genedesigns(CONTROL1, fullmatchcolumn=fullmatchcolumn, namecolumn=namecolumn, title=paste("% sgRNAs:", d.CONTROL1, sep=" "), xlab="% of sgRNAs present", ylab="# of Genes")

control2.readspergene = plot.reads.genedesigns(CONTROL2, fullmatchcolumn=fullmatchcolumn, namecolumn=namecolumn, title=paste("% sgRNAs:", d.CONTROL2, sep=" "), xlab="% of sgRNAs present", ylab="# of Genes")

treat1.readspergene = plot.reads.genedesigns(TREAT1, fullmatchcolumn=fullmatchcolumn, namecolumn=namecolumn, title=paste("% sgRNAs:", d.TREAT1, sep=" "), xlab="% of sgRNAs present", ylab="# of Genes")

treat2.readspergene = plot.reads.genedesigns(TREAT2, fullmatchcolumn=fullmatchcolumn, namecolumn=namecolumn, title=paste("% sgRNAs:", d.TREAT2, sep=" "), xlab="% of sgRNAs present", ylab="# of Genes")
...
write.xlsx(data.wilcox, file=paste(datapath, paste(analysis.name, "HIT-CALLING.xls", sep="_"), sep="/"), sheetName="Wilcox")
...
plot.waterfall.pval(type="wilcox", dataset=data.wilcox, pval=sig.pval.wilcox, log=TRUE)
```
...
if(nrow(data.wilcox.plot.enriched[data.wilcox.plot.enriched$p.value < sig.pval.wilcox,]) > 0)
  {
 kable(data.wilcox.plot.enriched[data.wilcox.plot.enriched$p.value < sig.pval.wilcox,])
...
`r kable(data.wilcox.plot.enriched[1:number.enriched.counter,])`
...
if(nrow(data.wilcox.plot.depleted[data.wilcox.plot.depleted$p.value < sig.pval.wilcox,]) > 0)
  {
 kable(data.wilcox.plot.depleted[data.wilcox.plot.depleted$p.value < sig.pval.wilcox,])
...
`r kable(data.wilcox.plot.depleted[1:number.depleted.counter,])`
...
# DESeq2
data.deseq = stat.DESeq(untreated.list = list(CONTROL1, CONTROL2), treated.list = list(TREAT1,TREAT2), namecolumn=namecolumn, fullmatchcolumn=fullmatchcolumn, extractpattern=g.extractpattern, sorting=FALSE, filename.deseq = paste(analysis.name, "-ANALYSIS-DESeq2-sgRNA.tab", sep=""), fitType="mean")

write.xlsx(as.data.frame(data.deseq$genes), file=paste(datapath, paste(analysis.name, "HIT-CALLING.xls", sep="_"), sep="/"), sheetName="DESeq2", append=TRUE)
...
plot.waterfall.pval(type="deseq2", dataset=data.deseq, pval=sig.pval.wilcox, log=TRUE)
```
...
if(nrow(data.deseq.plot.enriched[data.deseq.plot.enriched$padj < sig.pval.deseq,c(2,3,6,8)]) > 0)
  {
 kable(data.deseq.plot.enriched[data.deseq.plot.enriched$padj < sig.pval.deseq,c(2,3,6,8)])
  } else { cat("No genes showed significant enrichment with a p-value lower than", sig.pval.deseq)}
```

According to the value set, the __top `r number.hits.plot.enriched` % enriched genes__ were:  

`r kable(data.deseq.plot.enriched[1:number.enriched.counter,c(2,3,6,8)])`
...
if(nrow(data.deseq.plot.depleted[data.deseq.plot.depleted$padj < sig.pval.deseq,c(2,3,6,8)]) > 0)
  {
 kable(data.deseq.plot.depleted[data.deseq.plot.depleted$padj < sig.pval.deseq,c(2,3,6,8)])
...
`r kable(data.deseq.plot.depleted[1:number.depleted.counter,c(2,3,6,8)])`
...
write.xlsx(as.data.frame(data.mageck$genes), file=paste(datapath, paste(analysis.name, "HIT-CALLING.xls", sep="_"), sep="/"), sheetName="MAGeCK", append=TRUE)
...
plot.waterfall.pval(type="mageck", dataset=data.mageck, pval=sig.pval.wilcox, log=TRUE)
```
...
if(nrow(data.mageck.plot.enriched) > 0)
  {
 kable(data.mageck.plot.enriched[,c(2,3,4,5,6,7)])
...
`r kable(data.mageck.plot.enriched[1:number.enriched.counter,c(2,3,4,5,6,7)])`
...
if(nrow(data.mageck.plot.depleted) > 0)
  {
  kable(data.mageck.plot.depleted[,c(2,3,4,5,6,7)])
...
`r kable(data.mageck.plot.depleted[1:number.depleted.counter,c(2,3,4,5,6,7)])`
...
plot.hit.overview(wilcox=data.wilcox, deseq=data.deseq, mageck=data.mageck, title="Hit Overview", cutoff.deseq = sig.pval.deseq, cutoff.wilcox = sig.pval.wilcox, cutoff.mageck = sig.pval.mageck, cutoff.override=cutoff.override, cutoff.hits=NULL, plot.genes="overlapping")
...
if(nrow(overlap.enriched.dataset) > 0)
  {
    ...
    kable(overlap.enriched.dataset)
...
```{r HT-compare-enriched-overlap, echo=FALSE,sanitize=TRUE}
...
venn.diagram.crispr(venn.enriched, fill=c("lightgreen","lightblue2","lightgray"), na="remove")
```
...
if(nrow(overlap.depleted.dataset) > 0)
  {
    ...
    kable(overlap.depleted.dataset)
...
```{r HT-compare-depleted-overlap, echo=FALSE,sanitize=TRUE}
...
venn.diagram.crispr(venn.depleted, fill=c("lightgreen","lightblue2","lightgray"), na="remove")
```
...
```{r HT-compare-overlap-enriched-scatter, echo=FALSE, sanitize=TRUE,fig.width=8,fig.height=8}
# make Hit Scatter with all HITS enriched in a 4x4 square

plot.read.count.vs(dataset=list(CONTROL1.g, CONTROL2.g,TREAT1.g, TREAT2.g), dataset.names = c(d.CONTROL1, d.CONTROL2, d.TREAT1, d.TREAT2), pairs=FALSE, namecolumn=namecolumn, fullmatchcolumn=fullmatchcolumn, title=analysis.name, pch=16, normalize=normalize, norm.function=norm.function, labelgenes=overlap.enriched, labelcolor="orange", center=FALSE, aggregated=TRUE)
```
...
```{r HT-compare-overlap-depleted-scatter, echo=FALSE, sanitize=TRUE,fig.width=8,fig.height=8}

plot.read.count.vs(dataset=list(CONTROL1.g, CONTROL2.g,TREAT1.g, TREAT2.g), dataset.names = c(d.CONTROL1, d.CONTROL2, d.TREAT1, d.TREAT2), pairs=FALSE, namecolumn=namecolumn, fullmatchcolumn=fullmatchcolumn, title=analysis.name, pch=16, normalize=normalize, norm.function=norm.function, labelgenes=overlap.depleted, labelcolor="orange", center=FALSE, aggregated=TRUE)
```
...
write.xlsx(final.tab, paste(datapath, paste(analysis.name, "FINAL.xls", sep="_"), sep="/"), sheetName="Genelist", 
...
#write.xlsx(final.tab, paste(datapath, paste(analysis.name, "FINAL.xls", sep="_"), sep="/"), sheetName="All", 
...
for(i in overlap.enriched)
  {
    ...
    if(count==0)
      {
      write.xlsx(overlap.enriched, file=paste(datapath, paste(analysis.name, paste("HITS-sgRNA", paste("enriched", "xls", sep="."), sep="-"), sep="_"), sep="/"), sheetName="List of Genes", row.names=FALSE, col.names=FALSE)
      }
    
  sgrnas.en.table = plot.sgrna.table(wilcox=data.wilcox, deseq=data.deseq, mageck=data.mageck, dataset=list(CONTROL1, CONTROL2, TREAT1, TREAT2), dataset.names = c(d.CONTROL1, d.CONTROL2, d.TREAT1, d.TREAT2), namecolumn=namecolumn, fullmatchcolumn=fullmatchcolumn, norm.function=norm.function, extractpattern=g.extractpattern, type="enriched", labelgenes=i, cutoff.deseq = sig.pval.deseq, cutoff.wilcox=sig.pval.wilcox, cutoff.mageck = sig.pval.mageck, cutoff.override=cutoff.override.enriched, cutoff.hits=number.hits.plot.enriched, sgrna.file = libFILE, write=FALSE)

  write.xlsx(sgrnas.en.table, file=paste(datapath, paste(analysis.name, paste("HITS-sgRNA", paste("enriched", "xls", sep="."), sep="-"), sep="_"), sep="/"), sheetName=as.character(i), append=TRUE, row.names=FALSE)
        
  
print(kable(sgrnas.en.table))
  cat("\n")
...
for(i in overlap.depleted)
 ...
 if(count==0)
      {
      write.xlsx(overlap.depleted, file=paste(datapath, paste(analysis.name, paste("HITS-sgRNA", paste("depleted", "xls", sep="."), sep="-"), sep="_"), sep="/"), sheetName="List of Genes", row.names=FALSE, col.names=FALSE)
      }
 
  sgrnas.dep.table = plot.sgrna.table(wilcox=data.wilcox, deseq=data.deseq, mageck=data.mageck, dataset=list(CONTROL1, CONTROL2, TREAT1, TREAT2), dataset.names = c(d.CONTROL1, d.CONTROL2, d.TREAT1, d.TREAT2), namecolumn=namecolumn, fullmatchcolumn=fullmatchcolumn, norm.function=norm.function, extractpattern=g.extractpattern, type="depleted", labelgenes=i, cutoff.deseq = sig.pval.deseq, cutoff.wilcox=sig.pval.wilcox, cutoff.mageck = sig.pval.mageck, cutoff.override=cutoff.override.depleted, cutoff.hits=number.hits.plot.enriched, sgrna.file = libFILE, write=FALSE)
 
   write.xlsx(sgrnas.dep.table, file=paste(datapath, paste(analysis.name, paste("HITS-sgRNA", paste("depleted", "xls", sep="."), sep="-"), sep="_"), sep="/"), sheetName=as.character(i), append=TRUE, row.names=FALSE )

print(kable(sgrnas.dep.table))
...
write.xlsx(data.analysis.enriched, file=paste(datapath, paste(analysis.name, "COMPARE-HITS.xls", sep="_"), sep="/"), sheetName="Enriched")
kable(data.analysis.enriched[,c(2:7)])
```
...
```{r HT-3d-enriched, echo=FALSE, sanitize=TRUE}
#,fig.width=10,fig.height=10
par(mfrow=c(1,1))
compared = compare.analysis( wilcox=data.wilcox, deseq=data.deseq, mageck=data.mageck, type="enriched", cutoff.deseq = sig.pval.deseq, cutoff.wilcox = sig.pval.wilcox, cutoff.mageck = sig.pval.mageck, cutoff.override=TRUE, cutoff.hits=number.enriched.counter, output="3dplot", sort.by=c("mageck","rank","rank"), plot.method=c("wilcox","deseq","mageck"), plot.feature=c("pval","pval","fdr"))
...
write.xlsx(data.analysis.depleted, file=paste(datapath, paste(analysis.name, "COMPARE-HITS.xls", sep="_"), sep="/"), sheetName="Depleted", append=TRUE)

kable(data.analysis.depleted[,c(2:7)])
```
...
```{r HT-3d-depleted, echo=FALSE, sanitize=TRUE}
par(mfrow=c(1,1))
compared2 = compare.analysis( wilcox=data.wilcox, deseq=data.deseq, mageck=data.mageck, type="depleted",  cutoff.deseq = sig.pval.deseq, cutoff.wilcox = sig.pval.wilcox, cutoff.mageck = sig.pval.mageck, cutoff.override=TRUE, cutoff.hits=number.depleted.counter, output="3dplot", sort.by=c("mageck","rank","rank"), plot.method=c("wilcox","deseq","mageck"), plot.feature=c("pval","pval","fdr"))
...
_Significantly_ enriched genes, which had a corrected p-value below the threshold set for each method, are highlighted in __red__.  
_Significantly_ depleted genes, which had a corrected p-value below the threshold set for each method, are highlighted in __blue__.  

Moreover, the number of significant sgRNAs is presented for MAGeCK as well.

### Wilcox

```{r HT-visualization-wilcox, echo=FALSE, sanitize=TRUE}
#, fig.height=10, fig.width=15
wilcox.result = plot.hitident(data.wilcox, type="wilcox", title=paste(analysis.name, "wilcox", sep=" "), xlab="Genes", ylab="log10(p-value)", print.names=TRUE, pch=16,  xlim=NULL, ylim=c(2,20), offsetplot=1.2, plot.p=sig.pval.wilcox, separate=TRUE)
...
```{r HT-visualization-deseq, echo=FALSE, sanitize=TRUE}
#, fig.height=10, fig.width=15
deseq.result = plot.hitident(data.deseq, type="deseq2", title=paste(analysis.name, "DESeq2", sep=" "), xlab="Genes", ylab="log10(p-value)", print.names=TRUE, pch=16,  xlim=NULL, ylim=c(2,20), plot.p=sig.pval.deseq, separate=TRUE, offsetplot=1.2, sgRNA.top=sgRNAs.top.deseq)
...
```{r HT-visualization-mageck, echo=FALSE, sanitize=TRUE}
#, fig.height=10, fig.width=15

mageck.result = plot.hitident(data.mageck, type="mageck", title=paste(analysis.name, "MAGeCK", sep=" "), xlab="", ylab="-log10(FDR)", pch=16, inches=0.1, print.names=TRUE, xlim=NULL, ylim=NULL, separate=TRUE, onedirection=FALSE, plot.p=sig.pval.mageck, offsetplot=1.2, sgRNA.top=sgRNAs.top.mageck)
...
if(identical(a.annotate.hits, TRUE))
  {
    ...
    cat("_", datapath,"_", "\n", "\n")
    cat("__", paste(analysis.name, "ANNOTATION.xls", sep="_"),"__", "\n")
  ...
  write.xlsx(annotate.enriched.all, file=paste(datapath, paste(analysis.name, "ANNOTATION.xls", sep="_"), sep="/"), sheetName="Enriched")
  ...
  write.xlsx(annotate.depleted.all, file=paste(datapath, paste(analysis.name, "ANNOTATION.xls", sep="_"), sep="/"), sheetName="Depleted", append=TRUE)
...
Bowtie2 Index File  | `r bowtie2file`
